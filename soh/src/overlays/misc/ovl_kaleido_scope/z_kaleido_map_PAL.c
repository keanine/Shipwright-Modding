...
void FastTravel_SetEntrance(s16 nextEntranceIndex) {
...
void FastTravel_TryTravel(PauseContext* pauseCtx, Input* input) {
    if (CHECK_BTN_ALL(input->press.button, BTN_A)) {
        ...
        FastTravel_SetEntrance(entranceIndex);
    }
}
...
void KaleidoScope_DrawWorldMap(PlayState* play, GraphicsContext* gfxCtx) {
    ...
    if ((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (pauseCtx->pageIndex == PAUSE_MAP)) {
        ...
        if (pauseCtx->cursorSpecialPos == 0) {
            ...
            FastTravel_TryTravel(pauseCtx, input);
            // MOD END
